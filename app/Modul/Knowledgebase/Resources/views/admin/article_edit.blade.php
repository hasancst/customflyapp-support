...
@section('judul', 'Edit Artikel Knowledge Base')

@section('konten')
<div class="card">
    <form action="/admin/kb/article/update/{{ $article->id }}" method="POST">
        @csrf
        <div style="display: grid; grid-template-columns: 1fr 300px; gap: 30px;">
            <div>
                <div style="margin-bottom: 20px;">
                    <label style="display: block; margin-bottom: 8px; font-weight: 600;">Judul Artikel</label>
                    <input type="text" name="judul" value="{{ $article->judul }}" required style="width: 100%; padding: 10px; border-radius: 8px; border: 1px solid #cbd5e1;">
                </div>

                <div style="margin-bottom: 20px;">
                    <label style="display: block; margin-bottom: 8px; font-weight: 600;">Konten Artikel</label>
                    <textarea name="konten" id="summernote" required>{{ $article->konten }}</textarea>
                </div>
            </div>
            ...
            <div>
                ...
                <div style="background: #f8fafc; padding: 20px; border-radius: 12px; border: 1px solid #e2e8f0; position: sticky; top: 100px;">
                    <div style="margin-bottom: 20px;">
                        ...
                        <select name="category_id" required style="width: 100%; padding: 10px; border-radius: 8px; border: 1px solid #cbd5e1;">
                            @foreach($categories as $c)
                                <option value="{{ $c->id }}" {{ $article->category_id == $c->id ? 'selected' : '' }}>{{ $c->nama }}</option>
                            @endforeach
                        </select>
                    </div>
                    ...
                    <div style="margin-bottom: 20px;">
                        ...
                        <input type="number" name="urutan" value="{{ $article->urutan }}" style="width: 100%; padding: 10px; border-radius: 8px; border: 1px solid #cbd5e1;">
                    </div>

                    <div style="margin-bottom: 20px;">
                        <label style="display: block; margin-bottom: 8px; font-weight: 600;">Tag (Pisahkan dengan koma)</label>
                        <input type="text" name="tags" value="{{ $article->tags }}" placeholder="panduan, help, setup" style="width: 100%; padding: 10px; border-radius: 8px; border: 1px solid #cbd5e1;">
                    </div>

                    <div style="margin-bottom: 25px;">
                        <label style="display: flex; align-items: center; gap: 10px; cursor: pointer;">
                            <input type="checkbox" name="aktif" {{ $article->aktif ? 'checked' : '' }} value="1">
                            <span>Publikasikan</span>
                        </label>
                    </div>

                    <button type="submit" class="btn" style="width: 100%; margin-bottom: 10px;">Update Artikel</button>
